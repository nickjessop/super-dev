...
📊 **Architecture viewer & canvas discussion**: interactive multi-diagram canvas with pan/zoom, live reload, side-by-side node inspector, bidirectional canvas commenting, and ADR history (`/arch`)
...
- **FigJam-style dynamic canvas navigation**: Two-finger trackpad panning with native momentum and diagonal freedom, Spacebar grab tool, and middle-click drag. Gentle, natural cursor-centered exponential pinch zoom (and <kbd>Cmd</kbd>/<kbd>Ctrl</kbd> + wheel) that eliminates jarring leaps. Intelligent aspect-ratio framing ensures tall vertical architectures (like pipeline flows) start comfortably readable and top-aligned without shrinking to a tiny ribbon.
...
- **Ephemeral local HTTP server & debounced live watcher**: Spawns an ephemeral Node.js HTTP server bound strictly to `127.0.0.1` with a 15-minute inactivity auto-shutdown and graceful process exit hooks. Watches markdown files with `fs.watch` debounced at 150ms. Real-time changes push to connected clients via Server-Sent Events (SSE), updating diagrams in real-time while preserving your active zoom and pan coordinates.
...
- **Context preservation via sub-agents**: When `/arch` is invoked during an active planning or coding session, the live review loop is delegated to an isolated sub-agent. The sub-agent manages the multi-turn discussion on the canvas and returns an executive briefing to the parent thread upon conclusion, keeping the main context window 100% clean for implementation.
